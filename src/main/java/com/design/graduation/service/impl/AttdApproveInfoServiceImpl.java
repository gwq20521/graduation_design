 ...
 * copyright: 2018
 */
package com.design.graduation.service.impl;
...
import com.design.graduation.dao.AttdApproveInfoMapper;
...
public class AttdApproveInfoServiceImpl implements AttdApproveInfoService {
    ...
    public JqGridJsonBean select(String page, String rows, String order_by, AttdApproveInfo attdApproveInfo) {
        ...
        try {
            ...
            List<AttdApproveInfo> data = attdApproveInfoMapper.selectData(attdApproveInfo, _rows, (_page - 1) * _rows,
                    order_by);
     ...
     */
    public JqGridJsonBean selectRelationData(String page, String rows, String order_by,
            AttdApproveInfo attdApproveInfo) {
        // TODO Auto-generated method stub
        JqGridJsonBean jgjb = new JqGridJsonBean();
        try {
            int _page = Integer.parseInt(page);
            ...
            //根据查询条件查询总页数
            int pages = (count % Integer.parseInt(rows)) == 0 ? (count / _rows) : ((count / _rows) + 1);
            List<Map<String, Object>> data = attdApproveInfoMapper.selectRelationData(attdApproveInfo, _rows,
                    (_page - 1) * _rows, order_by);
            jgjb.setPage(_page);// 第几页
            jgjb.setRecords(count);// 总数据量
            jgjb.setTotal(pages);// 总页数
            jgjb.setRoot(data);// 查询数据信息
